...
export const Board: React.FC = () => {
  ...
  const handleBoardDetected = (recognizedBoard: number[][]) => {
    setBoard(recognizedBoard);
    setReadonlyCells(Array(9).fill(Array(9).fill(true))); // Mark all cells as readonly
  };
  ...
  const generateNewPuzzle = (difficulty: 'easy' | 'medium' | 'hard') => {
    ...
    const { board: puzzleWithRemovedCells, readonlyCells: updatedReadonlyCells } =
      removeCells(newGeneratedBoard, difficulty);
    ...
    <div className="max-w-md mx-auto p-4 mt-8 bg-gray-100 rounded-lg shadow-lg">
      ...
      <div className="flex justify-between items-center mt-4 space-x-4">
        <IconButton
          ...
          color="text-white bg-emerald-600 hover:bg-emerald-500"
        ...
        <IconButton
          ...
          onClick={() => solvePuzzle(board, setBoard)}
        />
        <IconButton
          icon={<BiBulb />}
          reversed
          title="Hint"
          color="text-white bg-amber-600 hover:bg-amber-500"
          onClick={revealHint}
        />
      </div>
      ...
      <div className="max-w-md mx-auto p-4 mt-8">
        <ImageSolver onBoardDetected={handleBoardDetected} />
        {/* Test */}
        {/* {preprocessedPreview && (
          <div>
